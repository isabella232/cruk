(function ($) {
  ...
  Drupal.behaviors.cruk_justgiving = {};
  Drupal.behaviors.cruk_justgiving.attach = function(context, settings) {


  $("a.shortname-suggestion").click(function(e) {
    var shortname = $( "a.shortname-suggestion" ).text();
    $("#edit-just-giving-shortname" ).val(shortname);
    $("#jg-lookup" ).hide();
  });
    /*
     * Need to check the context otherwise the behaviour will be applied twice.  Context will be document on initial
     * page load, and when we do an ajax call, the context will be the form.  Anything else ignore.
     */
    if ( context != document && (!$(context).hasClass('webform-client-form') || !$(context).hasClass('pane-bundle-webform-thank-you-page'))) {
      return;
    }

    // temporary check for new reg forms containing JG injected via Drupal
    // ajax framework.  Disable button on submission is also handled in a separate
    // module
    if ($(context).find("form").hasClass('narrow-form')) {
      return;
    }
    ...
    if ( $('.jg-shortname').is('*') ) {
      ...
      var shortnameInput = $('input.jg-shortname');

      /*
       * On load make sure input box is ready only
       */

      $("#jgp-edit-link").click(function(e) {
        $(this).remove();
        e.preventDefault();
        shortnameInput
            .removeAttr('readonly')
      ...
      /*
       * Set the shortname input field to readonly and apply class to make it look
       * like text.  Only do this is the link is present, otherwise it will invoke this on
       * AJAX success on other components.
       */
      var editTriggered = $("#jgp-edit-link").is('*') ? false : true;
      ...
      var timer = null;
      shortnameInput.bind('keyup', function(e) {
        var elem = $(this);

        /*
         * Show the spinner straight away
         * Only showing if the 'edit' link also appears on the form
         * Drupal ajax framework will add the required markup for progress spinner
         */
        if ( !$('.lookup-spinner').is('*') && !editTriggered) {
          elem.after('<div class="ajax-progress ajax-progress-throbber lookup-spinner"><div class="throbber">&nbsp;</div></div>');
        }

        /*
         * Ignore keyup / blur if readonly.  Not sure why I need this but I do.
         */
      ...
      /* Drupal AJAX return / FAPI seems to be binding this more than once. Not sure how it does that */
      if ( !editTriggered ) {
        shortnameInput.bind('delayed-keyup', function(e) {

          e.preventDefault();
          if(!ajaxLoading) {

            var url = settings.cruk_justgiving.lookup_url,
                value = $(this).val(),
                element = $(this);
            ...
            $.ajax({
                ...
                beforeSend: function ( jqXHR, settings ) {
                  $("#jgp-lookup-link-result, ul#jg-suggestions").remove();
                },
                success: function (data, textStatus, xhr) {
                  $("div").remove(".ajax-progress");
                  if(data.valid)  {
                    element.parent().append('<p class="valid" id="jgp-lookup-link-result">'+ value +' is valid</p>');
                  } else if(data.suggestions.Names && data.suggestions.Names.length > 0)  {
                    element.parent().append('<p class="invalid" id="jgp-lookup-link-result">'+ value +' is already in use, why not try: <ul id="jg-suggestions" class="invalid"></ul></p>');
                    $.each( data.suggestions.Names, function( key, suggestion ) {
                      $('ul#jg-suggestions').append('<li><a rel="'+suggestion+'" class="jgp-lookup-suggestion" href="javascript:void(0)">'+ suggestion +'</a></li>');
                    });
                    $(".jgp-lookup-suggestion").click(function () {
                      var value = element.val($(this).attr("rel"));
                      $('ul#jg-suggestions, #jgp-lookup-link-result').remove();
                    });
                  } else {
                     element.parent().append('<p class="invalid" id="jgp-lookup-link-result">Please ensure your chosen shortname only contains characters, numbers and dashes</p>');
                  }
                  ajaxLoading = false;
                },
                error: function (xhr, textStatus, errorThrown) {
                  element.parent().append('<p class="invalid" id="jgp-lookup-link-result">There was an error contacting Just Giving, please try again</p>');
                  ajaxLoading = false;
                },
      ...
      $("#edit-submit, #edit-actions-submit, #edit-actions-next, #edit-next").bind('click', function(e) {
        /*
         * If AJAX is running on the page, prevent the form submission until it has finished
         */
        if ( jQuery.active ) {
          submitted = true;
        ...
        /**
         * Prevent submission if T&Cs box isn't checked
         */
        if ($('#edit-submitted-just-giving-terms-and-conditions-1').is(':visible') && !$('#edit-submitted-just-giving-terms-and-conditions-1').is(':checked')) {
            e.preventDefault();
            $('#edit-submitted-just-giving-terms-and-conditions .invalid').remove();
            $('#edit-submitted-just-giving-terms-and-conditions').append('<p class="invalid">Please accept JustGiving\'s terms and conditions</p>');
        }
      });


      $("#edit-submit, #edit-actions-submit, #edit-actions-next, #edit-next").bind("ajaxSend", function() {
        // disable button?
      }).bind("ajaxComplete", function() {

        if ( submitted ) {
          /*
           * If the button has been clicked and AJAX finished, submit the form.
           */
    ...
    // /*
    //  * Commenting this out as it conflicts with the clientside validation module
    //  * @TODO: add alternative 'please wait' underneath
    //  *
    //  * Ensure that the user doesn't try and use the submit button more than once
    //  *
    //  */
    // $('.webform-client-form').bind('submit', function() {
    //   $("#edit-actions-next,#edit-actions-submit").each(function(){
...
(function($) {
  ...
  $(document).ready(function() {
    ...
    if ($(".node .content .field.field-type-fivestar")[0]){
      // Remove widget when used in an existing node panel
      $( ".field.field-type-fivestar" ).remove();
    }
    // Opens a closes the rate this comment area.
    $('div.vote-comment-header').click(function() {
        $('div.vote-comment-area,div.vote-comment-submit-btn').toggle('fast');
    });

    $('div.fivestar-widget div.star').click(function() {
      $('div.star-vote-thank-you-msg').html('Thank you!').fadeIn(1000);
      $('#rate-this-form').addClass('stars-clicked');
    });
  });
})(jQuery);
  ...
  Drupal.behaviors.hideSubmitBlockit = {
    attach: function(context) {
      ...
      $('form', context).once('hideSubmitButton', function () {
        var $form = $(this);
        var submitButtonsSelector = 'input.form-submit, button.form-submit';

        if (typeof Drupal.settings.hide_submit.hide_submit_button_selector === 'string' &&
          Drupal.settings.hide_submit.hide_submit_button_selector.length) {
          submitButtonsSelector += ', ' + Drupal.settings.hide_submit.hide_submit_button_selector;
        }
        ...
        if (Drupal.settings.hide_submit.hide_submit_method === 'indicator') {
          ...
          $(submitButtonsSelector, $form).click(function (e) {
            var el = $(this);
        ...
        $('form', context).submit(function (e) {
          ...
          if (!e.isPropagationStopped()) {
            if (Drupal.settings.hide_submit.hide_submit_method === 'disable') {
              $(submitButtonsSelector, $form).attr('disabled', 'disabled').each(function (i) {
                var $button = $(this);
              ...
              if (Drupal.settings.hide_submit.hide_submit_hide_fx) {
                $(submitButtonsSelector, $form).addClass(Drupal.settings.hide_submit.hide_submit_css).fadeOut(100).eq(0).after(pdiv);
                $(submitButtonsSelector, $form).next().fadeIn(100);
              }
              else {
                $(submitButtonsSelector, $form).addClass(Drupal.settings.hide_submit.hide_submit_css).hide().eq(0).after(pdiv);
              }
            }
 ...
 * MIT / GPLv2 License.
 * Found at: https://github.com/scottjehl/iOS-Orientationchange-Fix/blob/99c9c99d58e9aa51fd8eef46c245c69f98830ed6/ios-orientationchange-fix.js
 */
(function(w){
...
CRUK.Mia = (function($, CRUK) {
    ...
    return {
        ...
        init: function() {
            ...
            CRUK.Mia.accordion();
            CRUK.Mia.GoogleTagManager();
            CRUK.Mia.FormRotationFix();
            CRUK.Mia.Aggregator.init();
            CRUK.Mia.initPdfLinks();
            CRUK.Mia.initSmsLinks();
        // CRUK.Mia.setDeviceBodyClass();
        // CRUK.Mia.cta();
        },

        // cta: function() {
        //   var heroHeight = $('.hero-section img').height().toString() + "px";
        //   $('.cta-section .blue-panel').height(ctaHeight);
        //   var ctaHeight= $('.cta-section .blue-panel').height().toString() + "px";
         ...
         * then changes the viewport scaling accordingly. This script fix the view port
         * rotation issue on ios devices.
         */
        FormRotationFix: function() {
            ...
            if (isForm > 0) {
                if (navigator.userAgent.match(/iPad/i)) {
                    ...
                    if (viewportmeta) {
                        viewportmeta.value = 'width=device-width, minimum-scale=1.0, maximum-scale=1.0, initial-scale=1.0';
                        document.addEventListener('touchstart', function() {
                            viewportmeta.value = 'viewportmeta.value = width=device-width, minimum-scale=0.50, maximum-scale=1.6';
                        ...
                        document.addEventListener('orientationchange', function() {
                            viewportmeta.value = 'viewportmeta.value = width=device-width, minimum-scale=1.0, maximum-scale=1.0, initial-scale=1.0';
                            setScrollBack = true;
                        }, false);
                    }
        ...
        GoogleTagManager: function() {
            ...
            var GOForm = '/support-us/do-your-own-fundraising/bbq/get-your-bbq-kit';
            var SU2C = '/stand-up-to-cancer';
            var element_id = '';
            var element_value = '';
            var form_path = '';
            var form_name = '';
            ...
            if (location === GOForm || location == SU2C) {
                form_path = location.split('/');
                form_name = form_path[form_path.length - 1];
                form_id = $('.node-registration-form form').attr('id');
                ...
                $('input[type=text]').focusout(function() {
                    element_value = $('#' + element_id).val();
                    if (element_value != '') {
                        _gaq.push(['_trackEvent', 'Form Field Tracking | ' + form_id, 'completed', element_id]);
                    } else {
                        _gaq.push(['_trackEvent', 'Form Field Tracking | ' + form_id, 'skipped', element_id]);
                    }
                });
                ...
                $('select').focusout(function() {
                    element_value = $('#' + element_id + ' option:selected').text();
                    element_value = element_value.replace('- Select -', '');
                    element_value = element_value.replace('Day', '');
                    element_value = element_value.replace('Month', '');
                    element_value = element_value.replace('Year', '');

                    if (element_value != '') {
                        _gaq.push(['_trackEvent', 'Form Field Tracking | ' + form_id, 'completed', element_id]);
                    } else if (element_value === '' || element_value === '- Select -') {
                        _gaq.push(['_trackEvent', 'Form Field Tracking | ' + form_id, 'skipped', element_id]);
                    }
                });
                ...
                $('input[type=checkbox]').focusout(function() {
                    is_check = $('#' + element_id).is(":checked");
                    if (is_check) {
                        element_value = 'Y';
                        _gaq.push(['_trackEvent', 'Form Field Tracking | ' + form_id, 'completed', element_id]);
                    } else {
        ...
        displayThankYouPage: function() {
            $('#e-activist-campaign-form.webform-client-form').find('.label').css('display', 'block');
        },
        ...
        // Accordion js used on scheme detail pages. Generic accordion panel uses js in collapse.js.
        accordion: function() {
        ...
        initHorizontalNav: function() {
            ...
            $('[class*="pane-menu-menu-horizontal-"]:first').attr('id', 'horizontal-nav-desktop').addClass('horizontal-nav--desktop')
            ...
            if ($desktopMenu.length) {
                $('body > div > div.wrapper').prepend('<div id="horizontal-nav-mobile" class="horizontal-nav--mobile"><div class="menu-toggle"><span class="menu-toggle-button">&#x025bc;</span><span class="menu-toggle-text">' + $menuTitle + '</span></div>');
            ...
            $parent = $('#horizontal-nav-desktop, #horizontal-nav-mobile').find('li.expanded');
            ...
            $('#horizontal-nav-desktop').find('li.expanded.level-1').hover(function() {
                var $this = $(this);
        ...
        gaShopMap: function() {
            ...
            if (map_url !== null) {
                ...
                $(window).bind("resize", function() {
                    ...
                    if (width <= 980) {
                        mobile_map_url = 'http://maps.googleapis.com/maps/api/staticmap?center=' + postcode[1] + '&zoom=16&size=320x233&markers=color:blue|label:S|' + postcode[1] + '&markers=size:tiny&format=jpg&sensor=false';
                        $('.pane-value #map-container img#static').attr('src', mobile_map_url);
                    } else if (width > 980) {
                        mobile_map_url = 'http://maps.googleapis.com/maps/api/staticmap?center=' + postcode[1] + '&zoom=16&size=620x350&markers=color:blue|label:S|' + postcode[1] + '&markers=size:tiny&format=jpg&sensor=false';
                        $('.pane-value #map-container img#static').attr('src', mobile_map_url);
                    }
                });
            }
        ...
        // Function to assing appropriate icon to email and telephone on the Group and Committee listing page.
                    ...
                    $(document).ready(function() {
                        if (width <= 640) {
                            ...
                            if ($('.views-field.search-email span').length > 0) {
                                $('.views-field.search-email span').parent().css('background-image', 'none');
                                $('.views-field.search-email span').parent().css('background-image', 'url(http://' + hostname + '/sites/all/themes/custom/cruk/images/telephone.png)');
                            }
                        } else {
        ...
        removeSchemeDateIcon: function() {
            var scheme_ref = $('.scheme-dates .view-scheme-detail-page-dates-view').find('div');
        ...
        DOMSetup: function() {
            ...
            if ($(".flag_day .white-multiple-panes .pane-fieldable-panels-pane").hasClass("multiple_blocks")) {
        ...
        winListeners: function() {
            $(window).load(function() {
                // $('.node-unpublished').css("background-color", "#f5f5f5");
                $('#e-activist-campaign-form').addClass('webform-client-form');
                CRUK.Mia.defaultNurseURL = $('#footer-promo .footer-promo-inner h2 a').attr('href');
                // CRUK.Mia.mobileLeftNav();
            ...
            //CRUK.Mia.setEqualColumnHeights('.panel-related-colum', '.panel-column');
            //CRUK.Mia.setDeviceBodyClass();
            });
            ...
            $(window).resize(function() {
                ...
                CRUK.Mia.setEqualHeights('.panel-related-row', '.panel-pane');
                CRUK.Mia.setEqualHeights('.two-content-pods-with-background-image-panel-pane .inner', '.pod .pod-inner');
            //CRUK.Mia.setEqualColumnHeights('.panel-related-column', '.panel-column');
            //CRUK.Mia.setDeviceBodyClass();
            });

            // $('#webform-client-form-6536').submit(function(event) {
            //   console.log(event);
            // });
        ...
        clickListeners: function() {
            ...
            $('.expandable-grid-panel h2').click(function() {
                $(this).toggleClass('collapsed').parent('.expandable-grid-panel').find('.pane-value').toggle('fast');
            });
            //end of expandable js

            // Show/hide collapsible sections (see old 'Fundraising Materials' standard value page).
            ...
            $('.info-links a').click(function() {

                action = $(this).text().toLowerCase();
                _gaq.push(['_trackEvent', 'core footer', 'footer link', action]);
            });
            ...
            $('.enewsletter a').click(function() {
                action = $(this).text().toLowerCase();
                _gaq.push(['_trackEvent', 'core footer', 'footer link', action]);
            });

            $('.pane-aggregator-feed-1 .item-list').click(function() {
                _gaq.push(['_trackEvent', 'Link', 'click', 'CRUK_Home | Blog_link']);
            });
            $('.pane-aggregator-feed-1 .blog-more-link').click(function() {
                _gaq.push(['_trackEvent', 'Link', 'click', 'CRUK_Home | Read more']);
            });
            $('.pane-aggregator-feed-1 .blog-press-release-link').click(function() {
                _gaq.push(['_trackEvent', 'Link', 'click', 'CRUK_Home | Press releases']);
            });

            $('#drupal-mdd .level3').click(function() {
                self = $(this);
                level2 = self.parents('.toplevel').children('p').children();
                level2_text = level2.text();

                // Handle more... header which doesn't have an 'a' object.
                if (level2_text == '') {
                    level2_text = 'More...';
                }
                level1 = self.parents('.mm-item').children('p').children();
                level1_text = get_mdd_string(level1.text());
                category = 'MDD_' + level1_text;
                action = level1_text + '_' + level2_text + '_' + self.text();
                _gaq.push(['_trackEvent', category, 'MegaDropdown', action]);
            });

            $('#drupal-mdd .level2').click(function() {
                self = $(this);
                level1 = self.parents('.mm-item').children('p').children();
                level1_text = get_mdd_string(level1.text());
                category = 'MDD_' + level1_text;
                action = level1_text + '_' + self.text();
                _gaq.push(['_trackEvent', category, 'MegaDropdown', action]);
            });

            $('#drupal-mdd .mm-item-link').click(function() {
                self = $(this);
                level1_text = get_mdd_string(self.text());
                category = 'MDD_' + level1_text;
                action = level1_text;
                _gaq.push(['_trackEvent', category, 'MegaDropdown', action]);
            });

            // Switch focus to free-text field on donate widget when appropriate radio checked.
            ...
            $('.donate-widget-option-wrapper').click(function() {
                ...
                if (clicked !== null) {
                    ...
                    if ($('#quicktabs-make_a_donation_quicktab .first').hasClass('active')) {
                        cloned = $('#quicktabs-tabpage-make_a_donation_quicktab-0 .donate-other-text-box').children().children().clone().removeAttr("id").addClass("donate-mobile-text-field").attr('id', 'mobile-donate-monthly-numberfield');
                        $('#quicktabs-tabpage-make_a_donation_quicktab-0 .donate-mobile-text').html(cloned);
                        ...
                        if ($('#mobile-donate-monthly-numberfield').css('display') != 'none') {
                            $('#mobile-donate-monthly-numberfield').focus();
                        }
                    } else {
                        cloned = $('#quicktabs-tabpage-make_a_donation_quicktab-1 .donate-other-text-box').children().children().clone().removeAttr("id").addClass("donate-mobile-text-field").attr('id', 'mobile-donate-single-numberfield');
                        $('#quicktabs-tabpage-make_a_donation_quicktab-1 .donate-mobile-text').html(cloned);
            ...
            // Home page tracking for it's cancer section.
            $('.left-bubble a').click(function() {
                _gaq.push(['_trackEvent', 'Link', 'click', 'CRUK_Home | '.concat($(this).text())]);
            });

            $('.right-bubble a').click(function() {
                _gaq.push(['_trackEvent', 'Link', 'click', 'CRUK_Home | '.concat($(this).text())]);
            });

            // GA tracking for reusable carousel.
            // Reusable carousel GA tracking for each slide caption.
            $('.carousel-caption').click(function(event) {
                var element_array = $('.carousel-caption').map(function() {
                    return $(this);
                });

                var cta_href = $('.item.image-item.active .carousel-caption').find('a').attr('href');
                for (var i = 0; i < element_array.length; i++) {
                    if (cta_href != null && element_array[i].find('a').attr('href') === cta_href) {
                        _gaq.push(['_trackEvent', 'Navigation', 'Carousel ', +(i + 1) + ' | ' + cta_href]);
                    }
                }
            });

            // Reusable carousel GA tracking for each slide button.
            function handlerMaker(i) {
                return function() {
                    _gaq.push(['_trackEvent', 'Navigation', 'Carousel', 'Button ' + i]);
                };
            }

            var $listButtons = $('.carousel-indicators li[title*=\"Show slide \"]');
            for (var i = 1; i <= $listButtons.length; i++) {
                $listButtons.eq(i - 1).click(handlerMaker(i));
            }

            // Reusable carousel right control GA tracking.
            $('.carousel-control-right').click(function() {
                _gaq.push(['_trackEvent', 'Navigation', 'Carousel', 'Right']);
            });

            // Reusable carousel left control GA tracking.
            $('.carousel-control-left').click(function() {
                _gaq.push(['_trackEvent', 'Navigation', 'Carousel', 'Left']);
            });


            // Reuseable Accordion GA tracking.
            $(".panel-title a").click(function(event) {
                if ($(this).attr('class') === 'collapsed') {
                    _gaq.push(['_trackEvent', 'Navigation', 'Accordion', 'Open | ' + $(this).text()]);
                } else if ($(this).attr('class') === '') {
                    _gaq.push(['_trackEvent', 'Navigation', 'Accordion', 'Close | ' + $(this).text()]);
                }
            });
            ...
            // GA tracking for carousel TO REMOVE OLD
            // $('.flexslider a').click(function() {
            //   carouselId = $(this).attr("id").replace('carousel-link-', '');
            //   label = 'CRUK_Home | Carousel | '.concat(carouselId).concat(' | ').concat($(this).attr('href'));
            //   _gaq.push(['_trackEvent', 'Link', 'click', label]);
            // });

            $('.home-page-promo a').click(function() {
                sourceURL = $(this).attr('href');
                if ($(this).text() === 'Our research' || $(this).text() === 'See our progress') {
                    _gaq.push(['_trackEvent', 'Link', 'click', 'CRUK_Home | Scientific facts promo | '.concat(sourceURL)]);
                }
                if ($(this).text() === 'Spotting cancer early could save your life' || $(this).text() === 'Learn more') {
                    _gaq.push(['_trackEvent', 'Link', 'click', 'CRUK_Home |  Case study promo | '.concat(sourceURL)]);
                }
                if ($(this).text() === 'Find a grant') {
                    _gaq.push(['_trackEvent', 'Link', 'click', 'CRUK_Home |  CRUK_Home | Find a grant']);
                }
                if ($(this).text() === 'How to apply') {
                    _gaq.push(['_trackEvent', 'Link', 'click', 'CRUK_Home |  CRUK_Home | How to apply']);
                }
            });
        ...
        focusListeners: function() {
            ...
            // Switch checked radio button to one next to free textfield, when that field is focussed.
        ...
        submitListeners: function() {
            ...
            $('#quicktabs-tabpage-make_a_donation_quicktab-0').submit(function() {
                _gaq.push(['_trackEvent', 'Button', 'click', 'CRUK_Home | Start regular donation ']);
            });
            $('#quicktabs-tabpage-make_a_donation_quicktab-1').submit(function() {
                _gaq.push(['_trackEvent', 'Button', 'click', 'CRUK_Home | One off donation']);
            });
            ...
            $(document).ready(function() {
                ...
                if ($('.tabs.primary li').length > 0) {
                    ...
                    $('.tabs.primary li').each(function() {
                        if ($(this).children('a').html() === 'View<span class="element-invisible">(active tab)</span>' || $(this).children('a').html() === 'View' || $(this).children('a').html() === 'View published' || $(this).children('a').html() === 'View published<span class="element-invisible">(active tab)</span>') {
                            $(this).children('a').addClass('viewlink block');
                            $(this).prependTo('.admin-tabs');
                            $(this).wrap('<div id="view" class="admin-tab"></div>');
                        } else if ($(this).children('a').html() === 'Edit' || $(this).children('a').html() === 'New draft') {
                    ...
                    $('.admin-tabs').evenIfHidden(function() {
                        ...
                        $('div#admin-toolbar.vertical div.admin-blocks div.admin-block').css('top', tabActiveTop);
                    });
                }
            });
        },
    ...
    attach: function (context, settings) {
      $(document).ready(function(){
        ...
        if ($("#webform-component-enter-your-name").length) {
          $('#webform-component-enter-your-name input').on('input propertychange paste', function() {
            if($(this).val()!="") {
              $("input[type=submit].hide-submit-disable").removeAttr("disabled").removeClass("hide-submit-disable");
            }
          });
        }
...
CRUK.MobileRebrand = function() {
    ...
    var
        ...
        init = function() {
            ...
            //jQuery.get('/sites/default/files/mobile-menu.tpl.html', function(data){
            jQuery.get('/sites/all/themes/custom/cruk/menu/mobile-menu.tpl.html', function(data) {
